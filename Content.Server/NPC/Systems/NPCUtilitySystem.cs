...
public sealed class NPCUtilitySystem : EntitySystem
{
    ...
    private void Add(NPCBlackboard blackboard, HashSet<EntityUid> entities, UtilityQuery query)
    {
        ...
        switch (query)
        {
            ...
            case NearbyHostileShuttlesQuery shuttlesQuery:
                {
                    ...
                    CompanyComponent? ownCompanyComp = null;
                    if (ownGrid != null)
                        TryComp(ownGrid.Value, out ownCompanyComp);
                    ...
                    foreach (var (target, targetComp) in _lookup.GetEntitiesInRange<ShipNpcTargetComponent>(_transform.GetMapCoordinates(xform), shuttlesQuery.Range))
                    {
                        ...
                        // Lua
                        if (ownCompanyComp != null
                            && !string.IsNullOrEmpty(ownCompanyComp.CompanyName)
                            && targetGrid != null
                            && TryComp<CompanyComponent>(targetGrid.Value, out var targetCompanyComp)
                            && string.Equals(ownCompanyComp.CompanyName, targetCompanyComp.CompanyName, StringComparison.OrdinalIgnoreCase))
                        {
                            continue;
                        }
            ...
            case NearbyNonFactionShuttlesQuery nonFactionQuery: // Lua
                {
                    var xform = Transform(owner);
                    var ownGrid = xform.GridUid;
                    CompanyComponent? ownCompanyComp = null;
                    ...
                    foreach (var (target, targetComp) in _lookup.GetEntitiesInRange<ShipNpcTargetComponent>(_transform.GetMapCoordinates(xform), nonFactionQuery.Range))
                    {
                        if (target == owner)
                            continue;
                        ...
                        if (targetComp.NeedGrid && targetGrid == null ||
                            targetGrid == ownGrid ||
                            (_transform.GetWorldPosition(target) - _transform.GetWorldPosition(xform)).Length() > nonFactionQuery.Range ||
                            targetComp.NeedPower && !this.IsPowered(target, EntityManager) ||
                            targetGrid != null && _whitelistSystem.IsBlacklistPass(nonFactionQuery.Blacklist, targetGrid.Value))
                        {
                            continue;
                        }
                        if (ownCompanyComp != null
                        ...
                        if (nonFactionQuery.ExcludedCompanies.Count > 0
                            && targetGrid != null
                            && TryComp<CompanyComponent>(targetGrid.Value, out var gridCompanyComp))
                        {
                            var gridCompany = gridCompanyComp.CompanyName;

                            if (!string.IsNullOrEmpty(gridCompany)
                                && nonFactionQuery.ExcludedCompanies.Contains(gridCompany))
                            {
                                continue;
                            }
                        }
