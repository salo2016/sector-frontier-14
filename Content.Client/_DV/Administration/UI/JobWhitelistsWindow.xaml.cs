...
public sealed partial class JobWhitelistsWindow : FancyWindow
{
    ...
    public void HandleState(JobWhitelistsEuiState state)
    {
        ...
        var addedJobs = new HashSet<ProtoId<JobPrototype>>();
        var sortedDepartments = _proto.EnumeratePrototypes<DepartmentPrototype>()
            .Where(d => d.Roles.Count > 0).OrderBy(d => Loc.GetString(d.Name)).ToList();
        foreach (var proto in sortedDepartments)
        {
            var panel = new DepartmentWhitelistPanel(proto, _proto, state.Whitelists, state.GlobalWhitelist, addedJobs);
            panel.OnSetJob += (id, whitelisting) => OnSetJob?.Invoke(id, whitelisting);
            Departments.AddChild(panel);
        }
        // Frontier: ghost role prototypes
        GhostRoles.RemoveAllChildren();
        var ghostRoles = _proto.EnumeratePrototypes<GhostRolePrototype>()
            .Where(proto => proto.Whitelisted).OrderBy(proto => Loc.GetString(proto.Name)).Select(proto => new ProtoId<GhostRolePrototype>(proto.ID)).ToList();
